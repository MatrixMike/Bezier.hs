...
next dt world
  | animating world = world {time = time world + dt}
  | otherwise = world
...
touched (EventKey (SpecialKey KeySpace) Down _ _) world =
  world {animating = not (animating world)}
touched (EventKey (SpecialKey KeyDelete) Down _ _) world =
  world {points = init (points world)}
...
picture world =
  ...
  where
    ...
    go _ _ 0 _ = Blank
    go _ _ _ [_] = Blank
    go curveColor lineColor d ps =
      let go' = go (dim' curveColor) (dim' lineColor) (d - 1)
          a = bezier2d (init ps)
          b = bezier2d (tail ps)
          (ax, ay) = a t
          (bx, by) = b t
      in Pictures
           ...
           , Color lineColor $
             Translate (line1d ax bx t) (line1d ay by t) $ ThickCircle w r
           ]
